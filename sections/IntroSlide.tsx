  ...
  return (
    <section className="h-screen w-screen snap-start relative overflow-hidden flex items-center justify-center bg-[#f8f5ff]">
      ...
      <div className="container mx-auto px-6 relative z-10 max-w-5xl text-center">
        ...
        >
          De planning voor succes — in 8 weken van voorbereiding tot vlucht.
        </motion.p>

        <div className="grid md:grid-cols-4 gap-4">
          {[
            { icon: Mountain, label: 'Week 0', title: 'Nestfase', color: 'text-brand-accent', bg: 'bg-brand-accent' },
            { icon: Rocket, label: 'Week 1-2', title: 'Techniek', color: 'text-brand-green', bg: 'bg-brand-green' },
            { icon: Target, label: 'Week 3-4', title: 'Optimaliseren', color: 'text-cyan-400', bg: 'bg-cyan-400' },
            { icon: TrendingUp, label: 'Week 5-8', title: 'Testen & Live', color: 'text-brand-pink', bg: 'bg-brand-pink' },
          ].map((item, index) => (
            <motion.div
              key={item.label}
